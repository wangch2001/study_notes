...
研究动机？传统的训练需要标签，但标签定义过于繁琐和困难，如果能够在动作识别中处理大量无标记文本信息，且能够做zero shot

![image-20250521195407129](./assets/image-20250521195407129.png)

第一个改进：如何将图像变为视频，如何去计算相似度，与CLIP4Clip非常接近

第二个改进：使用了已标注的标签，因此当batch size比较大时，容易出现多个正样本

主体方法部分：

文本prompt：添加一些提示词，与prompt原本用法相同

视觉prompt：与之前的用法不同，更多的是作者的一些改进
