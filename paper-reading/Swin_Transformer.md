...
![image-20250513191554367](./assets/image-20250513191554367.png)
...
![image-20250513191624927](./assets/image-20250513191624927.png)

接着将所有的行标都乘上2M-1。

![image-20250513191635719](./assets/image-20250513191635719.png)

最后将行标和列标进行相加。这样即保证了相对位置关系，而且不会出现上述0 + ( − 1 ) = ( − 1 ) + 0 的问题了，是不是很神奇

![image-20250513191658374](./assets/image-20250513191658374.png)

刚刚上面也说了，之前计算的是相对位置索引，并不是相对位置偏执参数。真正使用到的可训练参数 $\hat{\mathrm{B}}$ 是保存在relative position bias table表里的，这个表的长度是等于( 2 M − 1 ) × ( 2 M − 1 ) 的。那么上述公式中的相对位置偏执参数B是根据上面的相对位置索引表根据查relative position bias table表得到的，如下图所示。

![image-20250513191807520](./assets/image-20250513191807520.png)
