...
# 二分类 BCE（pred是sigmoid后的概率）
def bce_loss(pred, target):
    loss = -(target * torch.log(pred) + (1 - target) * torch.log(1 - pred))
    return loss.mean()

# 多分类 CE (logits输入，labels是类别索引)
def cross_entropy_loss(pred, target):
    log_pred = F.log_softmax(pred)
    loss = log_pred[torch.arange(pred.size(0)), target]
    return -loss.mean()
...
class MoE(nn.Module):
    ...
    def forward(self, x):
        ...
        weights = F.softmax(topk_val, dim=-1)                 # 只对 Top-K 归一化

        # 分发 + 加权求和
        out = torch.zeros_like(x)
        for e, expert in enumerate(self.experts):
            tok = (topk_idx == e).any(dim=-1).nonzero(as_tuple=True)[0]
            if tok.numel() == 0:
                continue
            w = weights[tok][topk_idx[tok] == e].unsqueeze(-1)   # [n_e, 1]
            out[tok] += expert(x[tok]) * w
...
# 身高 x，体重 y
x = torch.tensor([160., 165., 170., 175., 180.])
y = torch.tensor([50., 55., 60., 65., 70.])
...
for epoch in range(epochs):
    ...
    if epoch % 1000 == 0:
        print(epoch, loss.item(), a.item(), b.item())
...
optimizer = torch.optim.SGD([a, b], lr=1e-5)
...
class TwoTower(nn.Module):
    def __init__(self, user_dim, item_dim, hidden_dim, emb_dim):
        super().__init__()
    ...
    def forward(self, user_x, item_x):
        user_emb = self.user_tower(user_x)   # [B, D]
        item_emb = self.item_tower(item_x)   # [B, D]
